...
PORT = 11405                # Sunucu port numarası
...
label_name = tk.Label(root, text="İsminiz:")
label_name.pack(pady=5)
entry_name = tk.Entry(root, width=30)
entry_name.pack(pady=5)

button_connect = tk.Button(root, text="Bağlan", command=connect_to_server)
...
button_send = tk.Button(root, text="Gönder", command=send_message)
button_send.pack(pady=5)

entry_message.bind("<Return>", send_message)

button_change_color = tk.Button(root, text="Arka Plan Rengini Değiştir", command=change_frame_color)
